...
function optimizar() {
  ...
  <div class="metrics-grid">
    <div class="metric"><div class="metric-label">Collares 💜</div><div class="metric-value">${x1}</div><div class="metric-unit">unidades</div></div>
    <div class="metric"><div class="metric-label">Manillas 💚</div><div class="metric-value">${x2}</div><div class="metric-unit">unidades</div></div>
    <div class="metric"><div class="metric-label">Anillos 🧡</div><div class="metric-value">${x3}</div><div class="metric-unit">unidades</div></div>
    <div class="metric"><div class="metric-label">Total piezas</div><div class="metric-value">${x1+x2+x3}</div><div class="metric-unit">al mes</div></div>
  </div>
  <div class="two-col">
    <div class="card">
      <div class="card-title">Uso de recursos</div>
      <div class="bar-row"><span class="bar-label">⏱ Tiempo</span><div class="bar-track"><div class="bar-fill" style="width:${pT}%;background:${barColor(pT)};"></div></div><span class="bar-pct">${pT}%</span><span class="bar-stats">${usedT}/${rT} min</span></div>
      <div class="bar-row"><span class="bar-label">🪡 Hilo</span><div class="bar-track"><div class="bar-fill" style="width:${pH}%;background:${barColor(pH)};"></div></div><span class="bar-pct">${pH}%</span><span class="bar-stats">${usedH}/${rH} cm</span></div>
      <div class="bar-row"><span class="bar-label">🟣 Perlas</span><div class="bar-track"><div class="bar-fill" style="width:${pP}%;background:${barColor(pP)};"></div></div><span class="bar-pct">${pP}%</span><span class="bar-stats">${usedP}/${rP} uni</span></div>
    </div>
    <div class="card">
      <div class="card-title">Recursos sobrantes</div>
      <div class="bar-row"><span class="bar-label">⏱ Tiempo</span><div class="bar-track"><div class="bar-fill" style="width:${100-pT}%;background:#c8d8c2;"></div></div><span class="bar-stats" style="font-weight:500;">${sobraT} min</span></div>
      <div class="bar-row"><span class="bar-label">🪡 Hilo</span><div class="bar-track"><div class="bar-fill" style="width:${100-pH}%;background:#c8d8c2;"></div></div><span class="bar-stats" style="font-weight:500;">${sobraH} cm</span></div>
      <div class="bar-row"><span class="bar-label">🟣 Perlas</span><div class="bar-track"><div class="bar-fill" style="width:${100-pP}%;background:#c8d8c2;"></div></div><span class="bar-stats" style="font-weight:500;">${sobraP} uni</span></div>
    </div>
  </div>
  <div class="two-col">
    <div class="card">
      <div class="card-title">Utilidad por producto (Bs/uni)</div>
      <div class="bar-row"><span class="bar-label">💜 Collar</span><div class="bar-track"><div class="bar-fill" style="width:${Math.max(0,u1/Math.max(u1,u2,u3,0.01)*100)}%;background:var(--purple);"></div></div><span class="bar-stats" style="font-weight:500;">Bs ${u1.toFixed(0)}</span></div>
      <div class="bar-row"><span class="bar-label">💚 Manilla</span><div class="bar-track"><div class="bar-fill" style="width:${Math.max(0,u2/Math.max(u1,u2,u3,0.01)*100)}%;background:var(--accent2);"></div></div><span class="bar-stats" style="font-weight:500;">Bs ${u2.toFixed(0)}</span></div>
      <div class="bar-row"><span class="bar-label">🧡 Anillo</span><div class="bar-track"><div class="bar-fill" style="width:${Math.max(0,u3/Math.max(u1,u2,u3,0.01)*100)}%;background:var(--gold);"></div></div><span class="bar-stats" style="font-weight:500;">Bs ${u3.toFixed(0)}</span></div>
    </div>
    <div class="card">
  ...
  <div class="recommendation">
    <div class="rec-header"><span class="rec-icon">💡</span><span class="rec-title">Recomendación del sistema</span></div>
    <div class="rec-body">
...
function renderSensibilidad() {
  ...
  const statusLabel = { critical: ['badge-critical','Limitante'], ok: ['badge-ok','Holgura'] };

  let rows = '';
  for (let i = 0; i < 3; i++) {
    const per100 = (d[i] * 100).toFixed(2);
    const [cls, lbl] = statusLabel[d[i] > 0.001 ? 'critical' : 'ok'];
    rows += `<tr>
      <td><strong>${resourceNames[i]}</strong></td>
      <td><span class="sens-badge ${cls}">${lbl}</span></td>
      <td>${d[i].toFixed(5)} Bs/${resourceUnits[i]}</td>
      <td class="${parseFloat(per100)>0?'plus':'zero'}" style="text-align:right;">
  ...
  <div class="card" style="margin-bottom:14px;">
    ...
    <div class="sens-table-wrapper">
      <table class="sens-table">
        <thead><tr><th>Recurso</th><th>Estado</th><th>Precio sombra</th><th style="text-align:right;">Impacto (+100 unidades)</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
  </div>
  <div class="recommendation" style="margin-bottom:14px;">
    <div class="rec-header"><span class="rec-icon">🎯</span><span class="rec-title">Recurso más crítico</span></div>
    <div class="rec-body">
  ...
  <div class="card">
    ...
    <div style="font-size:14px;color:var(--text2);margin-bottom:1rem;">Estima la nueva utilidad al agregar recursos adicionales.</div>
    <div class="sim-fields">
      <div class="sim-field"><label>+ Perlas (uni)</label><input type="number" id="simP" value="100" min="0"></div>
      <div class="sim-field"><label>+ Hilo (cm)</label><input type="number" id="simH" value="0" min="0"></div>
      <div class="sim-field"><label>+ Tiempo (min)</label><input type="number" id="simT" value="0" min="0"></div>
    </div>
    <button class="btn-sim" onclick="simular()">Simular impacto →</button>
    <div class="sim-result" id="sim-result">
      <div class="sim-result-inner">
        <div><div style="font-size:12px;color:var(--text3);margin-bottom:4px;">Nueva utilidad estimada</div><div class="sim-val" id="sim-val">—</div></div>
        <div><div style="font-size:12px;color:var(--text3);margin-bottom:4px;">Cambio vs. actual</div><div class="sim-diff" id="sim-diff">—</div></div>
      </div>
    </div>
  </div>`;
}
